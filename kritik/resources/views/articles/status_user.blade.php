<div class="col-md-6 col-md-offset-3"  >
			<div class="panel panel-default">
						...
						<ul class="list-unstyled list-inline" >
							...
							@if(\App\Statuslikes::where(['article_id'=>$article->article_id,'user_id'=>Auth::user()->id])->first())
							  ...
							  @else
								...
								<button class="btn btn-success btn-xs" onclick="like('{{$article->article_id}}')"><i class="fa fa-thumbs-up " aria-hidden="true"></i>Like Suggest</button>
				...
				<div class="panel-footer clearfix" style="background-color: white">	
					...
					{{--{{csrf_field()}}--}}
					{{--{{method_field('DELETE')}}--}}
						<button class="btn btn-danger btn-sm" onclick="deleteData({{$article->article_id}})">
							Delete
						</button>
					{{--</form>--}}
					{{-- {{Form::close()}} --}}
